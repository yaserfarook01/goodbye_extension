...
# File paths
CACHE_FILE = os.path.join(gettempdir(), "rule_based_tests_cache.json")

def load_cached_data():
    """Load cached test data from a file."""
...
def save_to_cache(data):
    """Save test data to a cache file."""
    with open(CACHE_FILE, 'w') as f:
        json.dump(data, f)

def fetch_all_rule_based_tests(endpoint, token, department_ids, branch_id="All", limit=25):
    """Fetch rule-based tests from the API, using a cache to avoid duplicate requests."""
    cached_data = load_cached_data()
    cached_test_ids = {test.get('testId') for test in cached_data}
    st.write(f"Loaded {len(cached_data)} tests from cache.")
    ...
    all_rule_based_tests = cached_data[:]
    page = 1
    ...
    while True:
        ...
        try:
            ...
            for test in tests:
                if test.get('testId') not in cached_test_ids:
                    all_rule_based_tests.append(test)
                    new_tests_added = True
            
            st.write(f"Page {page}: Fetched {len(tests)} tests, Added {len(all_rule_based_tests) - len(cached_data)} new tests.")
            page += 1
            time.sleep(0.5)
    ...
    if new_tests_added:
        save_to_cache(all_rule_based_tests)
        st.write("Updated cache with new tests.")
    return all_rule_based_tests

def extract_qb_names(tests):
    """Extract unique question bank names from the tests."""
    qb_names = set()
    
    for test in tests:
        test_rules = test.get('test_rules', [])
        if test_rules is not None:
            for rule_group in test_rules:
                for sub_group in rule_group:
                    for rule in sub_group:
                        for qb in rule.get('questionbank', []):
                            qb_name = qb.get('qb_name', 'Unknown QB')
                            if qb_name and qb_name != "No Question Banks Available":
                                qb_names.add(qb_name)
    
    return sorted(qb_names)

def generate_js_content(qb_names):
    """Generate the JavaScript content with QB names in a Set."""
    js_content = 'const qbNamesToHighlight = new Set([\n'
    for qb_name in qb_names:
        js_content += f'    "{qb_name}",\n'
    js_content += ']);\n'
    js_content += f'// Total QB names: {len(qb_names)}\n'
    return js_content

def main():
    st.title("QB Names Generator for Highlighter Extension")
    ...
    if not token:
        st.error("No API token provided. Please provide a token via the URL query parameter, e.g., ?token=your_token_here")
        return

    # API endpoint
    endpoint = "https://api.examly.io/api/v2/tests/filter"

    # Department IDs (update with your full list)
    department_ids = [
    ...
    with st.spinner("Fetching rule-based tests..."):
        tests = fetch_all_rule_based_tests(endpoint, token, department_ids)
        st.write(f"Total Rule Based Tests: {len(tests)}")

    # Extract QB names
    with st.spinner("Extracting QB names..."):
        qb_names = extract_qb_names(tests)
        st.write(f"Total unique QB names: {len(qb_names)}")

    # Generate JavaScript content
    js_content = generate_js_content(qb_names)

    # Display the JavaScript content (this will be fetched by the Chrome extension)
